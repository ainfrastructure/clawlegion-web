...
export function AgentFlowSection({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Preset Chips — wrapping grid with mini avatars */}
      <div className="space-y-2">
        <div className="flex flex-wrap gap-1.5">
          {DEFAULT_PRESETS.map((preset) => {
            ...
            return (
              <button
                ...
                className={`
                  relative flex items-center gap-2 pl-2 pr-2.5 py-1.5 rounded-lg text-[11px] font-semibold
                  transition-all duration-200 border
                  ${isSelected
                    ? 'bg-amber-500/15 border-amber-500/40 text-amber-400 shadow-[0_0_10px_-3px_rgba(245,158,11,0.2)]'
                    : 'bg-white/[0.03] border-white/[0.06] text-slate-400 hover:bg-white/[0.06] hover:border-white/[0.12] hover:text-slate-300'
                  }
                `}
              >
                {/* Mini avatar stack */}
                <div className="flex -space-x-1.5">
                  {presetAgents.slice(0, 3).map((a) => {
                    const agent = getAgentById(a.role)
                    return (
                      <div
                        key={a.role}
                        className="w-4 h-4 rounded-full overflow-hidden ring-1 ring-slate-800 flex-shrink-0"
                      >
                        {agent?.avatar ? (
                          <Image
                            src={agent.avatar}
                          ...
                          <div className="w-full h-full bg-slate-700 flex items-center justify-center">
                            <Bot className="w-2 h-2 text-slate-500" />
                          </div>
                        )}
                      </div>
                    )
                  })}
                  {presetAgents.length > 3 && (
                    <div className="w-4 h-4 rounded-full bg-slate-700 ring-1 ring-slate-800 flex items-center justify-center text-[7px] font-bold text-slate-400 flex-shrink-0">
                      +{presetAgents.length - 3}
                    </div>
                  )}
                </div>
          ...
          {selectedPresetId === 'custom' && (
            <div className="flex items-center gap-1.5 pl-2 pr-2.5 py-1.5 rounded-lg bg-amber-500/15 border border-amber-500/30">
              <Sparkles className="w-3 h-3 text-amber-400" />
              <span className="text-[11px] font-semibold text-amber-400">Custom</span>
            </div>
          )}
        </div>
        {/* Selected preset description */}
        {selectedPresetId !== 'custom' && (() => {
          const activePreset = DEFAULT_PRESETS.find(p => p.id === selectedPresetId)
          return activePreset ? (
            <p className="text-[11px] text-slate-500 leading-tight">{activePreset.description}</p>
          ) : null
        })()}
      </div>
