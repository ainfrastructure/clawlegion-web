...
export function AgentFlowSection({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Preset Chips — wrapping grid */}
      <div className="space-y-2">
        <div className="flex flex-wrap gap-1.5">
          {DEFAULT_PRESETS.map((preset) => {
            const isSelected = selectedPresetId === preset.id
            const presetAgentCount = preset.agents.filter(a => a.enabled).length
            return (
              <button
                key={preset.id}
                type="button"
                ...
                className={`
                  relative px-2.5 py-1.5 rounded-lg text-[11px] font-semibold
              ...
              >
                {preset.name}
                <span className={`ml-1 text-[9px] font-normal ${isSelected ? 'text-amber-500/60' : 'text-slate-600'}`}>
                  {presetAgentCount}
                </span>
              </button>
            )
          ...
          {selectedPresetId === 'custom' && (
            <div className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-amber-500/15 border border-amber-500/30">
