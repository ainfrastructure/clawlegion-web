...
import { X, Save, Loader2, Check, AlertCircle, BookOpen } from 'lucide-react'
import { cn } from '@/lib/utils'
import { useAgentsmd, useUpdateAgentsmd } from '@/hooks/useAgentConfig'

interface AgentsEditorModalProps {
  agentId: string
  agentName: string
  onClose: () => void
}

export function AgentsEditorModal({ agentId, agentName, onClose }: AgentsEditorModalProps) {
  const { data: agentsmdData, isLoading } = useAgentsmd(agentId)
  ...
  useEffect(() => {
    if (agentsmdData?.content) {
      setContent(agentsmdData.content)
      setHasChanges(false)
    }
  }, [agentsmdData])
  ...
  const handleContentChange = (newContent: string) => {
    ...
    setHasChanges(newContent !== agentsmdData?.content)
    setSaveStatus('idle')
  }
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      await updateAgentsmd.mutateAsync({
        agentId,
        content,
      })
      setSaveStatus('saved')
  ...
  }, [hasChanges, agentId, content, updateAgentsmd])
    ...
    <>
      ...
      <div className="fixed inset-4 md:inset-10 bg-slate-900 rounded-xl border border-white/[0.06] z-50 flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="flex-1 overflow-hidden">
          {isLoading ? (
            ...
            <textarea
              ...
              placeholder="# AGENTS.md - Workspace Rules..."
              spellCheck={false}
            />
          )}
        </div>
        ...
        <div className="px-4 py-2 border-t border-white/[0.06] bg-slate-800/50 flex items-center justify-between text-xs text-slate-500">
          <div>
            {agentsmdData?.path && (
              <span className="font-mono">{agentsmdData.path}</span>
            )}
          </div>
          <div className="flex items-center gap-4">
            <span>⌘S to save</span>
            <span>ESC to close</span>
            {agentsmdData?.lastModified && (
              <span>
                Last saved: {new Date(agentsmdData.lastModified).toLocaleString()}
              </span>
            )}
          </div>
        </div>
