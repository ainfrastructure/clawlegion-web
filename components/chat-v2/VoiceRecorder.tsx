...
interface VoiceRecorderProps {
  /** Called when voice recording is ready to send */
...
export function VoiceRecorder({
  ...
  const handleComplete = useCallback((blob: Blob, duration: number) => {
    ...
    const mimeType = getSupportedMimeType('voice') || 'audio/webm'
  ...
  // Idle state - start recording immediately or show button
  if (state === 'idle' && !recordingBlob) {
    // Auto-start recording when component mounts
    start()
    return (
      <div className="px-4 py-3 bg-slate-800/80 border-b border-white/[0.06]">
        <p className="text-sm text-slate-400">Starting voice recording...</p>
      </div>
    )
  }
